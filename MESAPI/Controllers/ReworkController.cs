...
{
    public class ReworkController : ApiController
    {

        [System.Web.Http.HttpPost()]
        public IHttpActionResult FPYRDataAll(ReWModel MoRework)
        {
            List<ReworkModel> _MatList = new List<ReworkModel>();
            ...
            else
            {
                ...
                string sql = @"select SERIAL_NUMBER,MO_NUMBER,MODEL_NAME,LINE_NAME,SECTION_NAME,GROUP_NAME,CASE to_number(ERROR_FLAG) WHEN 0 THEN 'PASS' end as RESULT,IN_STATION_TIME from r_sn_detail_t where serial_number in (select SERIAL_NUMBER from r_sn_detail_t where mo_number ='" + mo_number + "' and section_name = 'REWORK') ";
                ...
                if (dt.Rows.Count > 0)
                {
                    foreach (DataRow ds in dt.Rows)
                    {
                        ...
                        ReworkModel item = new ReworkModel();
